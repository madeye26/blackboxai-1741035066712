...
import { initializeChartLocale } from './charts.js';
...
const validationSchemas = {
    employee: {
        ...
        basic_salary: (value) => value && value > 0 ? null : 'الراتب الأساسي يجب أن يكون أكبر من صفر',
        job_title: (value) => value && value.trim().length > 0 ? null : 'المسمى الوظيفي مطلوب'
    }
};
...
async function initializeData() {
    try {
        ...
        appState.employees = employees.map(emp => ({
            ...
            work_days: emp.work_days || 22,
            daily_work_hours: emp.daily_work_hours || 8,
            monthly_incentives: emp.monthly_incentives || 0,
            dateAdded: emp.hire_date
        }));
        ...
        showAlert('خطأ في تحميل بيانات الموظفين', 'danger');
...
// Call initialize function when the application starts
...
function loadDashboard() {
    ...
    const totalSalaries = appState.employees.reduce((sum, emp) => sum + emp.basic_salary, 0);
...
function updateEmployeeInfo() {
    ...
    if (!employeeCode) {
        ...
        document.getElementById('monthly-incentives-display').value = '0';
...
async function handleEmployeeFormSubmit(e) {
    ...
    try {
        const formData = {
            ...
            work_days: parseInt(document.getElementById('work-days').value) || 22,
            daily_work_hours: parseInt(document.getElementById('daily-work-hours').value) || 8,
            basic_salary: parseFloat(document.getElementById('basic-salary').value),
            monthly_incentives: parseFloat(document.getElementById('monthly-incentives').value) || 0
        };
        ...
        const savedEmployee = await DatabaseService.createEmployee(employee);
...
function editEmployee(employeeCode) {
    ...
    document.getElementById('work-days').value = employee.work_days || 22;
    document.getElementById('daily-work-hours').value = employee.daily_work_hours || 8;
    document.getElementById('basic-salary').value = employee.basic_salary;
    document.getElementById('monthly-incentives').value = employee.monthly_incentives || 0;
